...
def test_answer_mode_applies_configured_provider_delay(tmp_path, monkeypatch):
    ...
    args = SimpleNamespace(
        ...
        answer_retry_failed_from=None,
        answer_max_retries=0,
        answer_retry_cooldown_seconds=0.0,
    )
...
def test_answer_mode_resumes_failed_queries_and_keeps_full_denominator(tmp_path, monkeypatch):
    payload = json.loads(_manifest(tmp_path, mode="synthetic").read_text(encoding="utf-8"))
    payload["queries"].append({
        **payload["queries"][0],
        "query_id": "q2",
    ...
    previous_report = {
        "answer": {
            "query_count": 2,
            "failure_count": 1,
            "non_empty_answer_rate": 0.5,
            "citation_presence_rate_required": 0.5,
            "average_expected_hint_overlap": 0.5,
            "estimated_tokens_used": 3,
            "queries": [
                {
                    "query_id": "q1",
                    "category": "smoke",
                ...
                },
                {
                    ...
                    "error": "HTTP 503: circuit breaker open",
                },
            ],
        }
    }
    previous_path = tmp_path / "previous_answer.json"
    ...
    args = SimpleNamespace(
        ...
        sec_metadata_weight=0.5,
        answer_disable_target_doc_filter=True,
        answer_delay_seconds=0.0,
        answer_retry_failed_from=previous_path,
        answer_max_retries=1,
        answer_retry_cooldown_seconds=2.5,
    )
    ...
    assert report["answer"]["query_count"] == 2
    assert report["answer"]["failure_count"] == 0
    assert report["answer"]["estimated_tokens_used"] == 8
    assert report["answer"]["queries"][0]["resume_retained_from_previous_report"] is True
    assert report["answer"]["queries"][1]["resume_previous_status"] == "failed"
    assert report["answer"]["queries"][1]["attempt_count"] == 2
    assert report["answer"]["resume"]["retry_query_count"] == 1
    assert report["answer"]["resume"]["source_metrics"]["failure_count"] == 1
    assert report["answer"]["resume"]["retry_metrics"]["failure_count"] == 0
